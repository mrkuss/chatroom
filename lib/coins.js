...
async function addCoins(username, amount) {
  // Track coins_earned as well as balance
  const r = await db.query(
    `UPDATE users SET coins = coins + $1, coins_earned = coins_earned + $1
...
async function deductCoins(username, amount) {
  // Track coins_spent as well as balance
  const r = await db.query(
    `UPDATE users SET coins = coins - $1, coins_spent = coins_spent + $1
     WHERE LOWER(username) = LOWER($2) AND coins >= $1 RETURNING coins`,
    [amount, username]
  );
