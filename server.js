...
app.post('/game/crash/start', async (req, res) => {
  if (!req.session.username) return res.status(401).json({ error: 'Not logged in' });
  const betAmount = parseInt(req.body.bet, 10);
  if (!betAmount || betAmount < 1 || betAmount > 500) return res.status(400).json({ error: 'Bet must be 1-500' });
  ...
  const rand = crypto.randomInt(1, 10000) / 100;
  let crashPoint;
  if (rand < 5) crashPoint = 1.0;
  else crashPoint = Math.max(1.0, parseFloat((100 / (rand - 4)).toFixed(2)));
  crashPoint = Math.min(crashPoint, 50);

  const key = req.session.username.toLowerCase();
  activeCrashGames[key] = { bet: betAmount, crashPoint, cashedOut: false, startTime: Date.now() };

  broadcastCoins(req.session.username, newBal);
  res.json({ ok: true, coins: newBal });
});

app.post('/game/crash/cashout', async (req, res) => {
  if (!req.session.username) return res.status(401).json({ error: 'Not logged in' });
  const key = req.session.username.toLowerCase();
  const game = activeCrashGames[key];
  if (!game) return res.status(400).json({ error: 'No active crash game' });
  if (game.cashedOut) return res.status(400).json({ error: 'Already cashed out' });

  const elapsed = (Date.now() - game.startTime) / 1000;
  const serverMultiplier = parseFloat(Math.min(Math.pow(1.07, elapsed * 10), game.crashPoint).toFixed(2));

  if (serverMultiplier >= game.crashPoint) {
    delete activeCrashGames[key];
    const coins = await getCoins(req.session.username);
    return res.json({ crashed: true, crashPoint: game.crashPoint, coins });
  }

  game.cashedOut = true;
  const winAmount = Math.floor(game.bet * serverMultiplier);
  const finalCoins = await addCoins(req.session.username, winAmount);
  delete activeCrashGames[key];
  broadcastCoins(req.session.username, finalCoins);
  res.json({ ok: true, multiplier: serverMultiplier, winAmount, coins: finalCoins });
});

app.post('/game/crash/result', async (req, res) => {
  if (!req.session.username) return res.status(401).json({ error: 'Not logged in' });
  const key = req.session.username.toLowerCase();
  const game = activeCrashGames[key];
  ...
  if (game.cashedOut) {
    delete activeCrashGames[key];
    const coins = await getCoins(req.session.username);
    return res.json({ alreadyCashedOut: true, coins });
  }
  const crashPoint = game.crashPoint;
  delete activeCrashGames[key];
  const coins = await getCoins(req.session.username);
  res.json({ crashed: true, crashPoint, coins });
});
